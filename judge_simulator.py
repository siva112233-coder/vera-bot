...
# Your API key — set via environment variable LLM_API_KEY (recommended)
LLM_API_KEY = os.environ.get("LLM_API_KEY", "")

# Model to use (leave empty for default, or specify like "gpt-4o", "claude-3-5-sonnet-20241022", etc.)
LLM_MODEL = "llama-3.1-8b-instant"  # <-- Optional: specify model or leave empty for default
...
# Force UTF-8 output on Windows — prevents UnicodeEncodeError with emoji in bot messages
...
class JudgeSimulator:
    ...
    def _warmup(self) -> bool:
        ...
        for slug, cat in self.dataset.categories.items():
            ...
            status = "PASS" if data and (data.get("accepted") or data.get("reason") == "stale_version") else "FAIL"
    ...
    def _full(self) -> bool:
        ...
        # Clear suppression and context state from warmup so full eval is clean
        ...
        # Tick all available triggers together to maximize merchant coverage
        ...
        actions = data.get("actions", [])
        print_info(f"Generated {len(actions)} actions across merchants ({lat:.0f}ms)")
